...
exports.getAllIssues = function(req, res, context, callback) {
  checkProject(context, req.params.project);
  var options = {
      host: 'api.github.com'
    , method: 'GET'
...
var checkProject = function(context, project) {
  console.log(project);
  var options = {
      host: 'api.github.com'
    , method: 'GET'
  ...
  var client = https.request(options, function(response) {
    ...
    response.on('end', function() {
      res = JSON.parse(res);
      Project.findOne({'github_id': res.id}, function(error, results) {
        if (results === null) {
          initProject(res.id, project);
        }
      });
    });
    response.on('error', function(error) {
...
var initProject = function(proj_id, proj_name) {
  console.log('initting cause it aint here');
