      ...
      client = https.request(options, function(response) {
        ...
        response.on('end', function() {
          ...
          checkProject(context, req.params.project, function(project_obj) {
            ...
            // closed = _.sortBy(closed, function(issue) {
            //   return issue.order;
            // })
...
var updateIssueState = function(req, res, issue, state, callback) {
  console.log(' - updateIssueState - ');  
  var post_data = '{"state": "'+state+'"}';
  console.log(post_data);
  var options = {
      host: 'api.github.com'
    , method: 'POST'
    , path: '/repos/stakes/Stylistic/issues/'+issue
    , headers: {
        'Content-Type': 'application/x-www-form-urlencoded'
      , 'Content-Length': post_data.length
      , 'Authorization': 'token '+req.session.auth.github.accessToken
...
var removeIssueLabel = function(req, res, issue, label) {
  var options = {
      host: 'api.github.com'
    , method: 'DELETE'
    , path: '/repos/stakes/Stylistic/issues/'+issue+'/labels/'+label
    , headers: {
        'Content-Length': '0'
      , 'Authorization': 'token '+req.session.auth.github.accessToken
    }
  ...
  var client = https.request(options, function(response) {
    ...
    response.on('end', function() {
      ...
      Project.findOne({'github_id': res.id}, function(error, result) {
        if (result === null) {
          initProject(res.id, project, callback);
        } else {
          callback(result);
        };
      });
...
var initProject = function(proj_id, proj_name, callback) {
  new Project({github_name: proj_name, github_id: proj_id}).save();
  result = Project.findOne({'github_id': proj_id}, function(error, result) {
    callback(result);
  })
};
