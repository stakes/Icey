...
exports.getAllIssues = function(req, res, context, callback) {
  ...
  var options = {
      host: 'api.github.com'
    , method: 'GET'
    , path: '/repos/'+req.session.context+'/'+req.session.project+'/issues?access_token=' + req.session.auth.github.accessToken
  };
    ...
    response.on('end', function() {
      ...
      client = https.request(options, function(response) {
        ...
        response.on('end', function() {
          ...
          checkProject(req.session.context, req.session.project, req.session.auth.github.accessToken, function(project_obj) {
