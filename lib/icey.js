...
exports.updateIssue = function(req, res, issue, label, state, callback) {
  ...
  var options = {
      host: 'api.github.com'
    , method: 'POST'
    , path: '/repos/'+req.session.context+'/'+req.session.project+'/issues/'+issue+'/labels'
    , headers: {
        'Content-Type': 'application/x-www-form-urlencoded'
      , 'Content-Length': post_data.length
      , 'Authorization': 'token '+req.session.auth.github.accessToken
...
var updateIssueState = function(req, res, issue, state, callback) {
  ...
  var options = {
      host: 'api.github.com'
    , method: 'POST'
    , path: '/repos/'+req.session.context+'/'+req.session.project+'/issues/'+issue
    , headers: {
        'Content-Type': 'application/x-www-form-urlencoded'
      , 'Content-Length': post_data.length
      , 'Authorization': 'token '+req.session.auth.github.accessToken
...
var removeIssueLabel = function(req, res, issue, label) {
  var options = {
      host: 'api.github.com'
    , method: 'DELETE'
    , path: '/repos/'+req.session.context+'/'+req.session.project+'/issues/'+issue+'/labels/'+label
    , headers: {
        'Content-Length': '0'
      , 'Authorization': 'token '+req.session.auth.github.accessToken
    }
...
var checkProject = function(context, project, callback) {
  var options = {
      host: 'api.github.com'
    , method: 'GET'
    , path: '/repos/'+context+'/'+project
  };
  var client = https.request(options, function(response) {
    var res = '';
    response.setEncoding('UTF8');
...
var checkLabels = function(proj_context, proj_name) {
  var options = {
      host: 'api.github.com'
    , method: 'GET'
  ...
  var client = https.request(options, function(response) {
    ...
    response.on('end', function() {
      ...
      // TODO: for each label, make sure it exists and if not create it
